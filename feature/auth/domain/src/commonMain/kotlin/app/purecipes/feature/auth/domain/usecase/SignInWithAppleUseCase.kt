...
class SignInWithAppleUseCase(
	...
	suspend operator fun invoke(profile: AppleAuthenticationProfile): Outcome<AuthUser> {
		if (profile.idToken.isBlank()) {
			return Err(Failure.ServerError("Apple sign-in did not return an ID token"))
		}
		return repository.signInWithApple(profile)
	}
}
