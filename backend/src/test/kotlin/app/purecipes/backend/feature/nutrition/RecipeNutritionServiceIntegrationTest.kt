...
class RecipeNutritionServiceIntegrationTest {
	...
	@Test
	fun calculateAndPersistDoesNotOverwriteScrapedNutrition() {
		assumeTrue(isDockerAvailable())

		val container = PostgreSQLContainer(DockerImageName.parse("postgres:17-alpine")).apply {
			withDatabaseName("purecipes_recipe_nutrition_scraped_test")
			withUsername("postgres")
			withPassword("postgres")
			start()
		}
		...
		try {
			...
			insertScrapedRecipeNutrition(
				dataSource = dataSource,
				recipeId = recipeId,
				calories = BigDecimal("256.00"),
			)

			RecipeNutritionService(dataSource).calculateAndPersist(recipeId)

			val calories = readRecipeCalories(dataSource, recipeId)
			calories shouldNotBe null
			calories!! shouldBeEqualComparingTo BigDecimal("256.00")
		} finally {
			dataSource.close()
			container.close()
		}
